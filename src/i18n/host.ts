...
export const zh: HostStrings = {
  ...
  'tool.render_chart.desc': '在聊天中渲染交互式 ECharts 可视化，并支持 HTML/PNG/CSV 导出。三个数据源，优先级：(1) 上次 run_sql 的 resultId — 即使只给你预览也能图表化完整结果；(2) sql — 在相同护栏下新鲜执行一条 SELECT；(3) 内联数据行。图表 option JSON 由平台构建 — 你只需声明意图：chartType、字段、标题。',
  'tool.render_chart.rendered': '图表已在对话中渲染（chartId {cid}，{points} 个数据点）。用户看到带 HTML/PNG/CSV 导出按钮的交互式图表。',
  'tool.analyze_data.desc': '对基础 SELECT 运行内置统计分析：profile（每列统计）、topn（分组 Top-N）、correlation（两数值列 Pearson 相关）、distribution（直方图）。传入基础 SQL；派生查询自动生成并执行。',
...
export const en: HostStrings = {
  ...
  'tool.analyze_data.desc': 'Run a built-in statistical analysis over a base SELECT: profile (per-column stats), topn (group-by top N), correlation (Pearson between two numeric columns), distribution (histogram). Pass the base SQL; derived queries are generated and executed for you.',
